...
#else
Process_p getpid( void )
{
	return FindProcess(NULL);
}
#endif
...
void __asm __saveds VSSL_CTX_Set_Options( __reg( a0, APTR ctx ), __reg( d0, ULONG options ) )
{
	SSL_CTX_set_options( ctx, options );
}

void __asm __saveds VSSL_Free_CTX( __reg( a0, APTR ctx ) )
{
	if( ctx )
		SSL_CTX_free( ctx );
}
...
APTR __asm __saveds VSSL_Connect( __reg( a0, APTR ctx ), __reg( d0, int sock ) )
{
	SSL *ssl = SSL_new( ctx );
...
void __asm __saveds VSSL_Close( __reg( a0, APTR ssl ) )
{
	SSL_free( ssl );
}

char * __asm __saveds VSSL_GetCipher( __reg( a0, APTR ssl ) )
{
	return( (char*)SSL_get_cipher( ssl ) );
}

int __asm __saveds VSSL_Write( __reg( a0, APTR ssl ), __reg( a1, APTR data ), __reg( d0, ULONG len ) )
{
	int rc;
...
int __asm __saveds VSSL_Read( __reg( a0, APTR ssl ), __reg( a1, APTR data ), __reg( d0, ULONG len ) )
{
	int rc;
...
int __asm __saveds VSSL_GetVerifyResult( __reg( a0, SSL *ssl ), __reg( a1, char **error ) )
{
...
X509 * __asm __saveds VSSL_GetPeerCertificate( __reg( a0, SSL *ssl ) )
{
...
X509_NAME * __asm __saveds VSSL_X509_get_subject_name( __reg( a0, X509 *cert ) )
{
	return( X509_get_subject_name( cert ) );
}

X509_NAME * __asm __saveds VSSL_X509_get_issuer_name( __reg( __a0, X509 *cert ) )
{
	return( X509_get_issuer_name( cert ) );
}

ASN1_UTCTIME * __asm __saveds VSSL_X509_get_notBefore( __reg( __a0, X509 *cert ) )
{
	return( X509_get_notBefore( cert ) );
}

ASN1_UTCTIME * __asm __saveds VSSL_X509_get_notAfter( __reg( __a0, X509 *cert ) )
{
	return( X509_get_notAfter( cert ) );
}

void __asm __saveds VSSL_AddCertDir( __reg( a0, SSL_CTX *ctx ), __reg( a1, STRPTR dir ) )
{
	//X509_add_cert_dir( ctx->cert, dir, X509_FILETYPE_PEM );
}

int __asm __saveds VSSL_WriteCertPEM( __reg( a0, X509 *cert ), __reg( a1, STRPTR outfile ) )
{
	BIO *out = NULL;
	int rc;
...
ULONG __asm __saveds VSSL_X509_NameHash( __reg( a0, X509 *cert ) )
{
	return( X509_subject_name_hash( cert ) );
}

int __asm __saveds VSSL_X509_HaveSubjectCert( __reg( a0, SSL_CTX *ctx ), __reg( a1, X509 *cert ) )
{
#if 0
	X509 *xi;
	int rc = FALSE;
...
int __asm __saveds VSSL_X509_fingerprint( __reg( a0, X509 *cert ), __reg( a1, UBYTE *md5 ) )
{
	unsigned int n = 0;
...
void __asm __saveds VSSL_X509_serialnumber( __reg( a0, X509 *cert ), __reg( a1, UBYTE *to ) )
{
...
void __asm __saveds VSSL_GetStats( 
	__reg( a0, APTR ctx ),
...
STRPTR __asm __saveds VSSL_GetVersion( __reg( a0, APTR ssl ) )
{
